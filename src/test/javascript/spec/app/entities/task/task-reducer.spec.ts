...
describe('Entities reducer tests', () => {
  ...
  describe('Requests', () => {
    it('should set state to loading', () => {
      testMultipleTypes([REQUEST(ACTION_TYPES.FETCH_TASK_LIST), REQUEST(ACTION_TYPES.FETCH_TASK)], {}, state => {
        expect(state).toMatchObject({
          errorMessage: null,
          updateSuccess: false,
          loading: true,
    ...
    it('should set state to updating', () => {
      testMultipleTypes(
        [REQUEST(ACTION_TYPES.CREATE_TASK), REQUEST(ACTION_TYPES.UPDATE_TASK), REQUEST(ACTION_TYPES.DELETE_TASK)],
        {},
        state => {
          expect(state).toMatchObject({
            errorMessage: null,
  ...
  describe('Failures', () => {
    it('should set a message in errorMessage', () => {
      testMultipleTypes(
        [
          FAILURE(ACTION_TYPES.FETCH_TASK_LIST),
          FAILURE(ACTION_TYPES.FETCH_TASK),
          FAILURE(ACTION_TYPES.CREATE_TASK),
          FAILURE(ACTION_TYPES.UPDATE_TASK),
          FAILURE(ACTION_TYPES.DELETE_TASK),
        ],
        'error message',
        state => {
          expect(state).toMatchObject({
  ...
  describe('Successes', () => {
    ...
    it('should fetch a single entity', () => {
      ...
      expect(
        reducer(undefined, {
          type: SUCCESS(ACTION_TYPES.FETCH_TASK),
          payload,
        })
      ).toEqual({
        ...initialState,
  ...
  describe('Actions', () => {
    ...
    it('dispatches ACTION_TYPES.FETCH_TASK_LIST actions', async () => {
    ...
    it('dispatches ACTION_TYPES.FETCH_TASK actions', async () => {
      const expectedActions = [
        {
          type: REQUEST(ACTION_TYPES.FETCH_TASK),
        },
        {
          type: SUCCESS(ACTION_TYPES.FETCH_TASK),
          payload: resolvedObject,
